...
rutaArchivos = 'C:/_Datos/02.Universidades/EAFIT/Cursos/2024-1/PCII/98.Ejemplos/Python/data/'
...
def mainApp():
    ...
    while True:
        try:
            ...
            ftxt = open(rutaArchivos + 'datosPCII.txt', 'a+')

            archivoCSV = rutaArchivos +'datosPCII.csv'

            if os.path.isfile(archivoCSV):
                fcsv = open(archivoCSV, 'a+')
            else:
                fcsv = open(archivoCSV, 'a+')
                headerList = ['Temperatura oC', 'Temperatura 0F', 'd3', 'd4', 'd5', 'd6'] 
